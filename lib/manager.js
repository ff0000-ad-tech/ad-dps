...
export const addSourceImageRequest = (dpsSource, { forCanvas } = {}) => {
	...
	const imageRequests = sources.map(source => {
		...
		if (dpsEnv === c.ENV_INLINED) {
			...
			// WARNING: image-id is generated from src filename...
			// If it collides with another requested filename, it will be incremented.
			// This may cause disparities between filename, import, and ImageManager.get(imageId)
			// when cs-plugin-dps-renderer bakes image-requests into the fba-payload.
			return ImageManager.getImageId(src)
		} else {
